 ...
 *  反之，从Linux上传文件到Windows，需要从UTF-8转码为GB2312
 */
...
int main()
{
	...
	//ftp地址
	string ftpSvrIp = "10.193.195.155";

	//ftp端口
	int port = 21;

	//用户名
	string userName = "LukeJia";

	//用户密码
	string password = "JBY250QMPZWSAD";

	//上传文件源路径
	string sourceFilePath = "D:/360MoveData/Users/lenovo/Desktop/upload";
	...
	do {
		...
		hftp = InternetConnect(hint, ftpSvrIp.c_str(), port, userName.c_str(), password.c_str(), INTERNET_SERVICE_FTP, INTERNET_FLAG_EXISTING_CONNECT || INTERNET_FLAG_PASSIVE, 0);
		if (hftp == NULL)
		{
			cout << "hftp出错" << endl;
			break;
		...
		if (FtpSetCurrentDirectory(hftp, desFilePath.c_str()))
		{
			...
			else
			{
				for (int i = 0; i < size; i++)
				{
					...
					if (!FtpPutFile(hftp, tempFilePath.c_str(), desfilename.c_str(), FTP_TRANSFER_TYPE_BINARY, 0))
						flag = false;
				}
			}
		}
		else
			cout << "设置ftp当前目录失败" << endl;
	} while (0);
